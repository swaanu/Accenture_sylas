...
Layer 4: Live Validation & False-Alarm Recalibration
  └── Real 80/20 Train / Holdout Split over live predictionLog entries
      └── Confusion matrix (TP, FP, FN, TN) on 80% train set + Out-of-Sample Accuracy, FAR, and Brier calibration on 20% unseen holdout slice
...
* **Pre-Populated Unit History**: On initialization, the simulation engine initializes with 500 completed unit operation cycles in \simEngine.predictionLog\ before the UI loads, and continuously pushes live completed operations.
* **80/20 Dataset Split**:
  * **80% Train Slice (400 samples)**: Used exclusively to evaluate the confidence threshold slider $\\tau \\in [10\\%, 90\\%]$, generating dynamic True Positives, False Positives, False Negatives, and True Negatives without fabricated fallbacks.
  * **20% Unseen Holdout Slice (100 samples)**: Reserved exclusively for evaluating out-of-sample forecast calibration, computing live unclamped Holdout Accuracy, False Alarm Rate, Brier Score, and ^2$.
* **Holdout-Gated Trust State**: The system trust state machine is gated on both in-sample accuracy and out-of-sample holdout calibration (^2 > 0.50$ and Holdout Accuracy $> 80\\%$ required for Trusted status).

### 3. Physics-Informed Real Solvers (PINN Models)
* **Live Solvers (4 Stations)**:
  * **Station S2 (Spot Weld Nugget Formation)**: Empirically calibrated lumped-parameter resistance spot weld (RSW) model fit to reference weld schedules per AWS D8.9M / ISO 18278-2 ((t) = \\sqrt{\\frac{k \\cdot I^2 \\cdot t}{\\rho \\cdot c_p}}$). Evaluated live across the full weld lobe spectrum ( \\approx 4.33\\text{ mm}$ under-current defect $\\to$ .08\\text{ mm}$ nominal $\\to$ .4\\text{ mm}$ expulsion defect).
...
* **Physics ^2$ Evaluation Scope**: Physics ^2$ is computed against a noise-perturbed version of the same physics model (no independent ground-truth telemetry exists in simulation) — it validates internal consistency of the noise model, not real-world predictive accuracy against physical hardware.
...
3. **Run Automated Test Assertions**:
   * **CLI Runner**:
     ...
     \   * **Direct Node.js Runner**:
     \\ash
     node tests/engine_assertions.test.js
     \   * **Interactive In-Browser Runner**:
     Open \http://localhost:8080/tests/test_runner.html4. **Keyboard Shortcuts**:
   * \ — Floor Supervisor (Real-time)
   * \ — Plant Manager (Weekly Trends)
   * \ — Leadership (Rollout & ROI)
   * \ — Modelling & Physics
